...
function Questions({ id }: IProps): JSX.Element {
  const renderQuestions = () => {
    switch (id) {
      ...
      case "2":
        return (
          <h2>
            Question{id}. 데이터를 처리하고 관리하는 데 관심이 많으신가요?
          </h2>
        );
      ...
      case "5":
        return (
          <h2>
            Question{id}. 웹사이트에서 동적인 콘텐츠와 애니메이션을 구현하는
            것에 관심이 있으신가요?
          </h2>
        );
      ...
      case "6":
        return (
          <h2>
            Question{id}. 알고리즘과 논리적 문제 해결에 관심이 많으신가요?
          </h2>
        );
      ...
      case "9":
        return (
          <h2>
            Question{id}. 반응형 웹 디자인과 크로스 브라우저 호환성 문제를 해결하는 데 관심이 있으신가요?
          </h2>
        );
  ...
  return <section className={styles.container}>{renderQuestions()}</section>;
}
